...
export class AIService {
  static async analyzeIdea(title: string, description: string): Promise<AIAnalysisResult> {
    try {
      const prompt = `
...
`;
  ...
  static async expandIdea(title: string, description: string): Promise<string[]> {
    try {
      const prompt = `
...
`;
  ...
  static async generateTasksFromMindMap(nodes: any[], edges: any[]): Promise<any[]> {
    try {
      const prompt = `
